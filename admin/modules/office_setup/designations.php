...
$edit = null;
if (isset($_GET['id'])) {
    $edit = $db->selectOne('SELECT * FROM `tbl_office_designation` WHERE `id` = ?', [(int) $_GET['id']]);
}
...
<div class="cms-drawer" id="formDrawer">
    <div class="cms-drawer-header">
        <h3><i class="fas fa-user-tie"></i><span id="drawerTitle"><?= $edit ? 'Edit' : 'Add' ?> Designation</span></h3>
...
function openDrawer(editId) {
    ...
    if (editId) {
        window.location.href = '<?= pageUrl('office_setup', 'designations') ?>&id=' + editId;
    }
}
